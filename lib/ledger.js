...
// Deterministic hash for event payload — used to verify signatures
function eventHash(event) {
  const payload = JSON.stringify({ type: event.type, data: event.data, ts: event.ts, id: event.id, author: event.author || '' });
  // Simple FNV-1a 32-bit hash — fast, deterministic, good enough for local verification
  let h = 0x811c9dc5;
...
// Verify event integrity — checks hash matches payload
...
function createEvent(type, data, signer) {
  ...
  // Event payload hash for signature verification
...
function applyEvent(state, event) {
  try {
    ...
    // Integrity check — reject tampered events
    if (event.hash && !verifyEventIntegrity(event)) {
      console.warn('Ledger: rejected tampered event', event.id);
      return state;
    }

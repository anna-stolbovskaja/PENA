// QVAC SDK — On-device receipt OCR (Tesseract.js) + natural language query engine
// All processing happens locally in the browser — no server, no API calls
// SDK interface: QVAC.init(), QVAC.parse(), QVAC.query(), QVAC.categorize(), QVAC.status()
...
const QVAC = {
  ...
  // Initialize OCR engine (lazy — call on first use)
  async init() {
    return initOCR();
  },

  // Parse receipt from image file/blob/URL
  async parse(imageInput) {
    return parseReceipt(imageInput);
  },

  // Query ledger with natural language
  query(state, queryText) {
    return queryLedger(state, queryText);
  },

  // Categorize expense text
  categorize(text) {
    return categorizeExpense(text);
  },

  // SDK status
  status() {
    return {
      ocrReady: tesseractWorker !== null,
      ocrLoading: tesseractLoading,
      version: this.version,
      capabilities: ['ocr', 'nlQuery', 'categorization'],
      supportedLanguages: ['spa', 'eng'],
    };
  },

  // Batch categorize multiple texts
  categorizeBatch(texts) {
    return (texts || []).map(t => ({
      text: t,
